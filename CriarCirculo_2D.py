...
for L in range(1,n_eletrodos):
    gmsh.model.geo.addCircleArc(L, ptoCentral, (L+1), L)
    #print('tag= L', L)    


gmsh.model.geo.addCircleArc(n_eletrodos, ptoCentral, 1, n_eletrodos)
# tag das linhas
tag_curvas = []
for i in range(0,n_eletrodos):
    tag_curvas.append(i+1)


print('tag_curvas',tag_curvas)
Loop = gmsh.model.geo.addCurveLoop(tag_curvas)
